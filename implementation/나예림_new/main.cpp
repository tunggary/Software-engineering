...
//나예림 상품 검색하기 추가
/*********************상품 검색하기************************/
//상품검색하기 boundary class : inputProductName()
void SearchProductUI::inputProductName(SearchProduct* searchProduct) {
    ...
    //2.1 showProductList()     SearchProduct의 showProductList
    searchProduct->showProductList(&productList, productName);
}

//상품검색하기 boundary class : printOutput()
void SearchProductUI::printOutput(ProductDetail* productDetails) {
    cout << "3. printOutput\n\n" << endl;
    //searchProductName = productDetails->productName();
    fout << "4.1. 상품 정보 검색" << endl;


    for (int i = 0; i < MAX_COUNT; i++) {
        string sellerId = productDetails[i].sellerId;
        string productName = productDetails[i].productName;
        string productCompanyName = productDetails[i].productCompanyName;
        int price = productDetails[i].price;
        int saleing = productDetails[i].saleing;
        float average = productDetails[i].average;
        fout << '>' << ' ' << sellerId << ' ' << productName << ' ' << productCompanyName << ' ' << price << ' ' << saleing << ' ' << average << endl;
    }
}

//상품구매내역조회하기 boundary class : printOutput()
